#include "custom_mutator_RISCV_falco.h"
    ...
    switch (opcode)
    {
        ...
        case I_TYPE_ENV:
            ...
            if (modified_inst.itype_inst.func3 == 0) {
                ...
                modified_inst.itype_inst.imm = mutate_value((int[]){0x0, 0x01, 0x2, 0x102, 0x302}, 5);
            // change csr command 
            } else if (value == 0) {
                modified_inst.itype_inst.rd = mutate_register();
            } else if (value == 1) {
    ...
    switch (instruction.base_inst.opcode)
    {
        ...
        //No wfi intstruction or sfence
            ...
            if (instruction.itype_inst.func3 == 0) {
                ...
                instruction.itype_inst.imm = mutate_value((int[]){0x0, 0x01, 0x2, 0x102, 0x302}, 5);
            } else { // csr
                instruction.itype_inst.rd = mutate_register();
                instruction.itype_inst.rs1 = mutate_register(); //uimm for csrrwi, uimm for csrrsi
