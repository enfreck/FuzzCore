#include "custom_mutator_RISCV_ridecore.h"

// Number of instructions in the initial seed
#ifndef _NUM_INSTRUCTIONS_CHARS
  #define _NUM_INSTRUCTIONS_CHARS 34
#endif
...
instruction_type mutate_field(instruction_type instruction) {
    ...
    switch (opcode)
    {
        ...
        //mulhu (func3 = 3, imm has 1 in lowest) 
            value = rand() % 5;
            if (value == 0) { // rd mutation
                ...
                //add (imm = 0), sub, (imm = 0x20), mul (imm = 0x1)
                if(modified_inst.rtype_inst.func3 == 0){
                    modified_inst.rtype_inst.func7 = mutate_value((int[]){0x0,0x20,0x1}, 3);
                }else if(modified_inst.rtype_inst.func3 == 1 || modified_inst.rtype_inst.func3 == 2 || modified_inst.rtype_inst.func3 == 3){
                    //mulh (imm = 0x1), sll (imm = 0), slt (imm = 0), mulhsu (imm = 0x1), sltu (imm = 0), mulhu (imm = 0x1)
                    modified_inst.rtype_inst.func7 = mutate_value((int[]){0x0,0x1}, 2);
                }else if(modified_inst.rtype_inst.func3 == 5){
                    //srl (imm = 0), sra (imm = 0x20)
                    modified_inst.rtype_inst.func7 = mutate_value((int[]){0x0,0x20}, 2);
                }else{
                    //func3: 4,6,7
                    //xor (imm = 0), //or (imm = 0), //and (imm = 0)
                    modified_inst.rtype_inst.func7 = 0;
                }
            }
            break;
        case I_TYPE_ARITH:
...
instruction_type mutate_instruction() {
    ...
    switch (instruction.base_inst.opcode)
    {
        ...
        // func7
            ...
            if(instruction.rtype_inst.func3 == 0){
            ...
            }else if(instruction.rtype_inst.func3 == 1 || instruction.rtype_inst.func3 == 2 || instruction.rtype_inst.func3 == 3){
                //mulh (imm = 0x1), sll (imm = 0), slt (imm = 0), mulhsu (imm = 0x1), sltu (imm = 0), mulhu (imm = 0x1)
                ...
                //func3: 4,6,7
                //xor (imm = 0), //or (imm = 0), //and (imm = 0)
                instruction.rtype_inst.func7 = 0;
            }
            break;
        case I_TYPE_ARITH:
        //addi (func3 = 0), slti (func3 = 2, sltiu (func3 = 3), xori (func3 = 4)
        ...
        // rd
                instruction.itype_inst.rd = mutate_register();
        // func3
                instruction.itype_inst.func3 = 2;
        // r1
                instruction.itype_inst.rs1 = mutate_register();
        // imm
                instruction.itype_inst.imm = mutate_imm();
        ...
        // func3
            instruction.stype_inst.func3 = 2;
        // r1
...
size_t afl_custom_fuzz(
    ...
    //4 instructions for 128 bits
    instruction_type insn_block[4];
    
    /*
    FILE* out_f = fopen("/tmp/curr_out.txt", "a");  // Open output file for writing
    if (!out_f) {
    ...
    */
    ...
    while (*curr) {
        ...
        //read block of 128 bits for 4 instructions
        int count = sscanf(curr, "%08x%08x%08x%08x %n", &(insn_block[0].raw_inst),&(insn_block[1].raw_inst), &(insn_block[2].raw_inst), &(insn_block[3].raw_inst), &offset);

        //If read in four, 32 bit instructions
        if (count == 4) {
            curr += offset;
            ...
            // str can hold 80 characters even though on 33 printed each cycle (32 for insn + space)
            char str[_NUM_INSTRUCTIONS_CHARS];
            out_pos += sprintf(str, "%08x%08x%08x%08x ", insn_block[0].raw_inst, insn_block[1].raw_inst, insn_block[2].raw_inst, insn_block[3].raw_inst);
