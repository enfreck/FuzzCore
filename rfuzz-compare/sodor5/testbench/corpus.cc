...
bool Corpus::is_interesting(Queue *q, unsigned long start){
    ...
    for(int i=0; i<N_COV_POINTS_b32; i++){
        uint32_t check = (~this->acc_output->coverage[i]) & new_output->coverage[i];  // is the corpus and q accumulated outputs different?
        if(check != 0){
            is_interesting = true;
            for(int j=0; j<32; j++){
                if(check & (1<<j)){
    ...
    if(is_interesting){
        ...
        num_input++;
        std::string filename = "./inputs/input" + std::to_string(num_input) + "_" + std::to_string(milliseconds_since_epoch-start) + ".txt";
        for (dinput_t* input : q->inputs){
            input->print_file(filename);
        }
    }
