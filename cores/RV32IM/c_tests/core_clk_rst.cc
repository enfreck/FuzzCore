...
int main (int argc, char** argv, char** env) {
	...
	for (int i = 0; i < ((CLOCK_CYCLES)*2); i++) { //32 more insn per run
        contextp->timeInc(1);
        top->clk = ~top->clk;
        top->eval();
	}
	...
	// for (auto a: rp->vlSymsp->TOP__rv32im_top__memory_modelling_inst.__PVT__imem) {
	// 	fprintf(complete_f, "@%x %x", a.first, a.second);
	// }
	// fprintf(complete_f,    "\n");
	// for (auto a: rp->vlSymsp->TOP__rv32im_top__memory_modelling_inst.__PVT__dmem) {
	// 	fprintf(complete_f, "@%x %x", a.first, a.second);
	// }
	// fprintf(complete_f, "\n-\n");
	// fclose(complete_f);
