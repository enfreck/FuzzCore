...
int num_queue_values_modified = 0;
...
static long fuzz(){
	...
	while(!corpus->empty()){
		...
		while(mutators->size()){
			...
			while(!mut->is_done()){
				...
				if(corpus->is_interesting(mut_q, milliseconds_since_epoch)){
					corpus->add_q(mut_q);
				}
				else{
					delete mut_q;
	...
	std::cout << "RFUZZ total number of cycles:" << tb->tick_count_ << std::endl;
	std::cout << "RFUZZ final coverage map: ";
	corpus->print_acc_coverage();
	std::cout << "RFUZZ sequences/sec: " << (double)(num_queue_values_modified * 20) / ((milliseconds_since_epoch_stop - milliseconds_since_epoch)/1000) << std::endl;
	...
	free(corpus->get_accumulated_output());
	delete corpus;
	delete rnd;
	delete tb;
