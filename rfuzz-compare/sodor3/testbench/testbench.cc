...
void Testbench::reset(){
    ...
    this->module_->reset = 0;
    this->module_->metaReset = 0;
    this->apply_vinput(inputs);

    this->tick(1);
    this->module_->reset = 1;
...
void Testbench::meta_reset(){
    ...
    this->module_->metaReset = 0;
    this->module_->reset = 0; // deassert normal reset while meta reset is running
    this->apply_vinput(inputs);
    this->tick(1);
    this->module_->metaReset = 1;
...
void Testbench::tick(int n_ticks, bool false_tick){
    for(int i=0; i<n_ticks; i++){
        this->tick_count_++;
        this->module_->clock = 0;
        this->module_->eval();
        #if VM_TRACE
        trace_->dump(5 * this->tick_count_ - 1);
        #endif // VM_TRACE
        this->module_->clock = !false_tick;
        this->module_->eval();
        #if VM_TRACE
