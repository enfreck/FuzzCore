...
int main(int argc, char **argv, char **env) {
	...
	Vrv32im_top* core = new Vrv32im_top{contextp};
	...
	for (int i = 0; i < NUM_CYCLES; i++)
	{
		...
		std::string icache_inst_w = line.substr(49, 32);      
		std::string intr_i = line.substr(81, 1);              
		std::string reset_vector_i = line.substr(82, 32);     
		std::string cpu_id_i = line.substr(114, 32);          
		...
		core->icache_inst_w = std::bitset<32>(icache_inst_w).to_ulong();
